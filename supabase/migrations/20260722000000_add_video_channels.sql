-- ==========================================
-- 20260722000000_add_video_channels.sql
-- コラボ動画サポート用中間テーブル video_channels の追加
-- ==========================================
...
CREATE TABLE IF NOT EXISTS public.video_channels (
  ...
  CONSTRAINT unique_video_channel UNIQUE (video_id, channel_id)
);
...
CREATE POLICY "Allow public read access on video_channels"
  ON public.video_channels FOR SELECT
  USING (true);

CREATE POLICY "Allow authenticated user insert on video_channels"
  ON public.video_channels FOR INSERT
  WITH CHECK (auth.uid() IS NOT NULL);

CREATE POLICY "Allow authenticated user delete on video_channels"
  ON public.video_channels FOR DELETE
  USING (auth.uid() IS NOT NULL);

-- 既存の videos の channel_record_id から video_channels を初期挿入
...
-- RPC: register_full_archive_transaction を拡張してコラボチャンネルIDリスト (p_collaborator_channel_ids) を受領可能にする
